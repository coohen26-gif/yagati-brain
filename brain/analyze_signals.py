...
def compute_rr(entry, stop, tp):
    try:
        ...
        stop = float(stop)
        tp = float(tp)
        # Calculate risk/reward ratio: abs((tp - entry) / (entry - stop))
        # Works for both LONG and SHORT positions due to abs()
        if entry == stop:
            return None
...
def main():
    ...
    for s in fetch_signals():
        ...
        opened.append({
            ...
            "decision": "KEEP",  # we actually opened the trade, so it's a KEEP
            "symbol": symbol,
